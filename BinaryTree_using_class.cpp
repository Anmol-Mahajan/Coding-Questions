...
public:
   ...
   void PrintInOrder(){///public function
    ...
    TreeNode* BuildPreOrderInOrder(int PreOrder[], int InOrder[], int sPO,
                        int ePO, int sIO, int eIO){
        if (sPO>ePO) return NULL;
        int lLST, lRST;
        int d=PreOrder[sPO]; int i;

        for (i=sIO;i<=eIO;i++){
            if (InOrder[i]==d) break;
        }
        lLST=i-sIO;
        lRST=ePO-sPO-lLST;
        TreeNode *rt=new TreeNode(d);
        rt->left=BuildPreOrderInOrder(PreOrder,InOrder,sPO+1,sPO+lLST,sIO,sIO+lLST-1);
        rt->right=BuildPreOrderInOrder(PreOrder,InOrder,sPO+lLST+1,ePO,i+1,eIO);
        return rt;
    }
